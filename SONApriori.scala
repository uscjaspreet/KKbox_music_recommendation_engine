...
object SONApriori {
  ...
  def case1(support: Int, ratingsFile: String, usersFile: String) {
    ...
    val phase2Result = baskets
      ...
      .collect()
    sc.stop
    using(new BufferedWriter(new OutputStreamWriter(new FileOutputStream("OutputFiles/Jaspreet_Singh_SON.case1_" + support + ".txt")))) {
      writer =>
        for (l <- phase2Result) {
  ...
  def case2(support: Int, ratingsFile: String, usersFile: String) {
    ...
    val phase2Result = baskets
      ...
      .collect()
    sc.stop
    using(new BufferedWriter(new OutputStreamWriter(new FileOutputStream("OutputFiles/Jaspreet_Singh_SON.case2_" + support + ".txt")))) {
      writer =>
        for (l <- phase2Result) {
          writer.write((sort(l._2.toSeq)).toString().replaceAll("List", "") + '\n') 
        }
